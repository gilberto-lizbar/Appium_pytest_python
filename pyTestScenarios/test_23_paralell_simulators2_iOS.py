...
def setup_function(request):
    ...
    appium_service.start(args=['--port', str(appium_port), '--base-path', '/'])
    print(f"Appium server started on port {appium_port}")
    ...
    if appium_service:
        try:
            if appium_service.is_running:
                print(f"Stopping Appium server on port {appium_port}...")
                appium_service.stop()
        except Exception as e:
            print(f"Error stopping Appium service: {e}")

    print(f"[Teardown] Cleanup complete for {request.param}.")
    # ****stopping appium service****>>
