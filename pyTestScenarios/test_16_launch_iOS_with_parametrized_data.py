...
def get_data():
    ...
    return [
        ("gilberto.lizbar@gmail.com", "Gilcarjuan$1725", "GilbertoLB"),
        ("gilberto.lizbar@gmail.com", "Gilcarjuan$1725", "GilbertoLB2")
    ]
...
@pytest.mark.parametrize("email, password, username", get_data())
def test_signUp_to_app(email, password, username):
    print("Sign Up test")
    driver.find_element(AppiumBy.ACCESSIBILITY_ID, "I already have an account").click()
    driver.find_element(AppiumBy.XPATH, "//*[@value='Enter email address']").send_keys(email)
    driver.find_element(AppiumBy.XPATH, "//*[@value='Enter password']").send_keys(password)
    driver.find_element(AppiumBy.ACCESSIBILITY_ID, "Continue").click()
    time.sleep(5)
    driver.find_element(AppiumBy.XPATH, "//*[@value='Enter username']").send_keys(username)
